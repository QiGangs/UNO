...
import com.qi.util.support.ErrorCode;
...
public class StringUtil {
	...
	public static String getRandomString(int length, boolean includeUpper, boolean includeLower, boolean includeDigital) {
		...
		String s = "";
		if (includeUpper) s += upper;
		if (includeLower) s += lower;
		if (includeDigital) s += digital;
		if (length > 100) throw new IllegalArgumentException("生成的字符串长度必须<100！");
