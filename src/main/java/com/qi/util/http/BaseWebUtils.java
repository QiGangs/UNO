...
public abstract class BaseWebUtils {
	...
	public static final boolean checkString(String str) {
		...
		if (StringUtils.contains(StringUtils.lowerCase(str), "<script")) return true;// 验证JS
		if (StringUtils.contains(StringUtils.lowerCase(str), "<iframe")) return true;// 验证iframe
		return false;
	}
		...
		try {
			...
			for (Object key : result.keySet()) {
				if (result.get(key) instanceof String) {
					if (checkString(result.get(key) + "")) return true;
				}
			}
		} catch (Exception ex) {
		}
